...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		/* if (SONG.song.toLowerCase() == 'massacre')
		{
			var chromosomes:Float = 15 / 5000;
		...
		*/
		...
		/* if (SONG.song.toLowerCase() == 'massacre')
		{
			var crackOverlay:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('city/brokenScreenOverlay'));
			crackOverlay.blend = BlendMode.ADD;
			crackOverlay.alpha = 0.65;
			add(crackOverlay);
			crackOverlay.cameras = [camOverlay];
		} */
	...
	private function generateSong(dataPath:String):Void
	{
		...
		/* if (ScarlettOptions.getWarningNotes())
		{
			var beatStepTime = 600*(100/songData.bpm);

			if (curSong == 'Russian-Rundown' && dad.curCharacter == 'velvet-rage')
			{
				for (x in 0 ... shootingBeat_RussianRundown.length)
				{
					var warnNoteTime = shootingBeat_RussianRundown[x];
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				...
				if (SONG.song.toLowerCase() == 'massacre')
				{
					FlxTween.tween(FlxG.camera, {zoom: 1.35}, 0.8, {ease: FlxEase.quadInOut});
				}
			}
			...
			if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100)
			{
				...
				if (SONG.song.toLowerCase() == 'massacre')
				{
					FlxTween.tween(FlxG.camera, {zoom: defaultCamZoom}, 0.8, {ease: FlxEase.quadInOut});
				}
			}
		}
	...
	override function beatHit()
	{
		...
		/* if (ScarlettOptions.getWarningNotes())
		{
			if (curSong == 'Russian-Rundown' && dad.curCharacter == 'velvet-rage')
			{
				if(shootingBeat_RussianRundown.contains(curBeat)){
					FlxG.sound.play(Paths.sound("velvetShoot"), 1);
